...
def lr_run_training(fold):
    ...
    pred_df.loc[valid_idx, f"rf_preds"] = valid_pred
...
if __name__ == '__main__':
    ...
    pred_df.to_csv('../preds/rf_preds.csv')

    if os.path.exists('./model_ckpt/rf/'):
        pass
    else:
        os.mkdir('./model_ckpt/rf/')
    
    for fold, model in enumerate(models):
        pickle.dump(model, open(f'./model_ckpt/rf/rf_{fold}.bin', 'wb'))
