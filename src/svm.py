...
def lr_run_training(fold):
    ...
    pred_df.loc[valid_idx, f"svm_preds"] = valid_pred
...
if __name__ == '__main__':
    ...
    pred_df.to_csv('../preds/svm_preds.csv')

    if os.path.exists('./model_ckpt/svm/'):
        pass
    else:
        os.mkdir('./model_ckpt/svm/')
    
    for fold, model in enumerate(models):
        pickle.dump(model, open(f'./model_ckpt/svm/svm_{fold}.bin', 'wb'))
