...
def lr_run_training(fold):
    ...
    pred_df.loc[valid_idx, f"ridge_preds"] = valid_pred
...
if __name__ == '__main__':
    ...
    pred_df.to_csv('../preds/ridge_preds.csv')

    if os.path.exists('./model_ckpt/ridge/'):
        pass
    else:
        os.mkdir('./model_ckpt/ridge/')
    
    for fold, model in enumerate(models):
        pickle.dump(model, open(f'./model_ckpt/ridge/ridge_{fold}.bin', 'wb'))
