...
def lr_run_training(fold):
    ...
    pred_df.loc[valid_idx, f"xgb_preds"] = valid_pred
...
if __name__ == '__main__':
    ...
    pred_df.to_csv('../preds/xgb_preds.csv')

    if os.path.exists('./model_ckpt/xgb/'):
        pass
    else:
        os.mkdir('./model_ckpt/xgb/')
    
    for fold, model in enumerate(models):
        pickle.dump(model, open(f'./model_ckpt/xgb/xgb_{fold}.bin', 'wb'))
