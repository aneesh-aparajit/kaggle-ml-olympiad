...
def lr_run_training(fold):
    ...
    pred_df.loc[valid_idx, f"lasso_preds"] = valid_pred
...
if __name__ == '__main__':
    ...
    pred_df.to_csv('../preds/lasso_preds.csv')

    if os.path.exists('./model_ckpt/lasso/'):
        pass
    else:
        os.mkdir('./model_ckpt/lasso/')
    
    for fold, model in enumerate(models):
        pickle.dump(model, open(f'./model_ckpt/lasso/lasso_{fold}.bin', 'wb'))
