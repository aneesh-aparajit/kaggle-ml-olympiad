...
import pickle
import os
from best_params import CATBOOST
...
def run_training(fold):
    ...
    preds_df.loc[valid_df.index, "cat_preds"] = preds

    return model, preds_df[["id", "result", "kfold", "cat_preds"]]
...
if __name__ == '__main__':
    ...
    if os.path.exists('../models/cat/'):
        pass
    else:
        os.mkdir('../models/cat/')

    for fold, model in enumerate(models):
        pickle.dump(model, open(f'../models/cat/cat-{fold}.bin', 'wb'))
    
    print(df.shape)
    df.to_csv('../preds/cat_preds.csv', index=False)

    print(df.head())
    print(np.sqrt(metrics.mean_squared_log_error(y_true=df['result'], y_pred=df['cat_preds'])))
