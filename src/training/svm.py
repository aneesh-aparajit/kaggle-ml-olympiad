...
def run_training(fold):
    ...
    preds_df.loc[valid_df.index, "svm_preds"] = preds

    return model, preds_df[["id", "result", "kfold", "svm_preds"]]
...
if __name__ == '__main__':
    ...
    if os.path.exists('../models/svm/'):
        pass
    else:
        os.mkdir('../models/svm/')

    for fold, model in enumerate(models):
        pickle.dump(model, open(f'../models/svm/svm-{fold}.bin', 'wb'))
    
    print(df.shape)
    df.to_csv('../preds/svm_preds.csv', index=False)

    print(df.head())
    print(np.sqrt(metrics.mean_squared_log_error(y_true=df['result'], y_pred=df['svm_preds'])))
