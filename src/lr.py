...
def lr_run_training(fold):
    ...
    pred_df.loc[valid_idx, f"lr_preds"] = valid_pred
...
if __name__ == '__main__':
    ...
    pred_df.to_csv('../preds/lr_preds.csv')

    if os.path.exists('./model_ckpt/lr/'):
        pass
    else:
        os.mkdir('./model_ckpt/lr/')
    
    for fold, model in enumerate(models):
        pickle.dump(model, open(f'./model_ckpt/lr/lr_{fold}.bin', 'wb'))
