...
if __name__ == '__main__':
    ...
    pred_df.to_csv('../preds/catboost_preds.csv')

    if os.path.exists('./model_ckpt/catboost/'):
        pass
    else:
        os.mkdir('./model_ckpt/catboost/')
    
    for fold, model in enumerate(models):
        pickle.dump(model, open(f'./model_ckpt/catboost/catboost_{fold}.bin', 'wb'))
